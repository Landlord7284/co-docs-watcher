...
`run` is one-shot: it does a complete pass and exits, printing a one-line-per-step summary of
what it did. Schedule it with whatever you already have — cron, launchd, a shell loop — or use
the container below, which carries its own scheduler. The usual pairing is a frequent
`run --monitor` plus one daily `run`.
