...
Unattended operation is a container that schedules itself, and it lives beside the package rather than inside it: `Dockerfile`, `compose.yaml`, `docker/entrypoint.sh`, `docker/run-profile.sh` and `.env.example`. Nothing under `src/` knows any of it exists, and what the scheduler fires is the same one-shot a shell runs.
