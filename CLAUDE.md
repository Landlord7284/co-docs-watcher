...
Unattended operation is a container that schedules itself, and it lives beside the package rather than inside it: `Dockerfile`, `compose.yaml`, `docker/entrypoint.sh`, `docker/run-profile.sh`, `docker/config-timezone.py` and `.env.example`. Nothing under `src/` knows any of it exists, and what the scheduler fires is the same one-shot a shell runs.
...
**There is one configuration file, and the container mounts it.** `config.toml` is the project's, not the host's: the container is shaped like a checkout — the file at `/watcher/config.toml` and the three host directories mounted at `/watcher/var/data`, `/watcher/var/documents` and `/watcher/var/logs`, which is where its relative roots resolve. A second, container-side TOML is refused: what it would duplicate is not the paths but everything else — `retention`, `discovery`, `files`, `source` — and two copies of a value that must not differ are a way for it to differ. The split is by question instead: `.env` answers where the directories are on the host and when each profile fires, `config.toml` answers how far back, how long, and with which modes, identically on both sides of the mount.
