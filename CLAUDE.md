...
| `source.fre_reading_pdf` | `false` | whether a run asks the source to render the reading PDF an FRE container does not carry |
...
- **Schedules live in the environment, windows live in the configuration file.** They answer different questions — how often to look, and how far back — and a change of cadence must not read like a change of coverage. The crontab carries a profile name and never a number, exactly as the `run` flag does. The cadence shipped is the monitor at midnight and hourly from 09:00 to 23:00, and the full sweep daily at 08:00: about 39 listing requests a day, which neither the image nor the entrypoint may bring closer to `source.min_request_interval` or `source.max_requests_per_run`.
...
│   ├── reading_pdf.py the session-bound chain that renders what a container omits
...
- **A reading copy the watcher asked for is filed exactly like one the container carried.** It lands in the category subfolder under the imposed name, marked unstable, and nothing downstream distinguishes it: where the PDF came from is a fact about the request, not about the delivery. A container that already carries one is never asked a second time.
...
- **One PageMethod for search**: `POST …/frmConsultaExternaCVM.aspx/ListarDocumentos`, JSON in, no session, no cookies, no `__VIEWSTATE` — and the download is the same, so the two paths the watcher lives on are stateless. The reading-copy chain below is the one exception, and it is one because the source made it one. `empresa` is a comma-separated list of six-digit zero-padded CVM codes **with a leading comma**; empty means the whole market. `dataDe`/`dataAte` (`dd/MM/yyyy`, both inclusive, only read with `periodo: "2"`) filter by **delivery date**.
...
- **A container does not always carry the reading copy, and the one that does not can be asked for it.** An ITR package holds the generated PDF; an FRE package holds the structured form and `FormularioCadastral.xml` and nothing a person reads (measured 2026-08-28 on `161120` v3 and `161107` v4). The readable version is what field 10's `OpenPopUpVer('frmGerenciaPaginaFRE.aspx?…')` opens, and it costs five requests: the listing page, the viewer shell, the generator shell, then `CarregarMenuRelatorios` and `GerarRelatorio` (continued by `ContinuarLeituraRelatorio` until it says it is finished). The generator refuses a session that did not walk in through the listing page, so the walk *is* the credential and every hop names the one before it. What comes back is a fixed 16 MiB buffer with the document at the front and NUL behind it: `BytesLidos` reports the buffer and `TamanhoTotal` reports `0`, so the end is found at the last `%%EOF` and nowhere else. Measured 2026-08-28: ~38 s for a 463-page, 11.1 MB PDF. `source.fre_reading_pdf` is off by default — the filing is archived either way, and this buys a PDF beside the XMLs at five requests a document.
- **The reading copy is captcha-gated by a switch the source owns.** `hdnHabilitaCaptcha` in the viewer shell says whether the page attaches a reCAPTCHA token, and `GerarRelatorio` can answer `V2: true` to demand an interactive one mid-chain. Both are `SolicitarCaptcha: "S"` in a different vocabulary and take the same reaction: stop, exit `4`, reduce frequency.
